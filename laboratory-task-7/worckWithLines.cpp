...
void getText(char* text,const char* textToWrite) {
    ...
    if (std::strlen(text) == 0) throw std::exception("Error line is empty...");
}
...
bool IsPalindrom(char* word) {
    ...
    for (size_t i = 0; i < lenWord / 2; ++i) {
        if (word[i] != word[lenWord - 1 - i]) return false;
    }
    return true;
}
...
size_t seekMaxLenPolindrom(char* text, char* delimiters) {
    ...
    size_t maxLen = 0;
    while (word != NULL) {
        if (maxLen < std::strlen(word) && IsPalindrom(word)) maxLen = std::strlen(word);
