...
void getText(std::string& line, const char* parametr = "") {
 ...
 std::getline(std::cin, line);
  if (line.size() == 0) throw std::xception("Line is empty...");
 bool isFullSpace = true;
 for (size_t i = 0; i < line.size(); ++i) {
  if (!isspace(line[i])) isFullSpace = false;
 }
 if (isFullSpace && (strcmp(parametr, "line") == 0)) throw std::exception("The string should not consist only of spaces");
}
...
bool isArmstrogNumber(std::string word) {
 if (isNaturallNumeric(word)) {
  ...
  if (number == sumNumbers) { return true; }
  else { return false; }
 }
 else {
  return false;
...
void findArmstrongsWords(std::string line, std::string delimiters, std::string& ArmstrongNumbers) {
 ...
 while (indexFinishCharctar < line.size()) {
  ...
  if (indexFinishCharctar == line.npos) indexFinishCharctar = line.length();
 ...
 if (lineDontHaveArmstrongNamber) ArmstrongNumbers = "Armstrong numbers not founded...";
}